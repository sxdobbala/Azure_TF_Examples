...
describe 'example_simple_bucket_without_policy' do
  ...
  context "example_simple_bucket_without_policy" do
    before (:all) do
      FileUtils.rm_rf Dir.glob("#{Dir.pwd}/spec/fixtures" )
      FileUtils.mkdir_p "#{Dir.pwd}/spec/fixtures"
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/simple_bucket_without_policy/*"],"#{Dir.pwd}/spec/fixtures", )
    end
    ...
    describe 'Verify terraform plan for example simple_bucket_without_policy' do
      before (:all) do
        terraform_plan_output
        @output = terraform_show
      end

      it 'Verify terraform plan for example simple_bucket_without_policy is successful' do
        ...
        expect( @output ).to match(/\+ module.simple-bucket-without-policy.aws_s3_bucket.bucket/)
        expect( @output ).to match(/id:\s+<computed>/)
        ...
        expect( @output ).to match(/\+ module.simple-bucket-without-policy.aws_s3_bucket_policy.bucket_policy/)
        expect( @output ).to match(/id:\s+<computed>/)
