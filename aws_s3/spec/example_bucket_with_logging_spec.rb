...
describe 'example_bucket_with_logging' do
  ...
  context "example_bucket_with_logging" do
    before (:all) do
      FileUtils.rm_rf Dir.glob("#{Dir.pwd}/spec/fixtures" )
      FileUtils.mkdir_p "#{Dir.pwd}/spec/fixtures"
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/bucket_with_logging/*"],"#{Dir.pwd}/spec/fixtures", )
    end
    ...
    describe 'Verify terraform plan for example bucket_with_logging' do
      before (:all) do
        terraform_plan_output
        @output = terraform_show
      end

      it 'Verify terraform plan for example bucket_with_logging is successful' do
        ...
        expect( @output ).to match(/logging.~4016003484.target_bucket:\s+"\${aws_s3_bucket.log_bucket.id}"/)
        expect( @output ).to match(/logging.~4016003484.target_prefix:\s+"logs\/"/)
        ...
        expect( @output ).to match(/\+ module.bucket-with-logging.aws_s3_bucket.log_bucket/)
        expect( @output ).to match(/id:\s+<computed>/)
        expect( @output ).to match(/acceleration_status:\s+<computed>/)
