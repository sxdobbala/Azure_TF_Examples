...
describe 'example_bucket_with_logging_and_tflife' do
  ...
  context "example_bucket_with_logging_and_tflife" do
    before (:all) do
      FileUtils.rm_rf Dir.glob("#{Dir.pwd}/spec/fixtures" )
      FileUtils.mkdir_p "#{Dir.pwd}/spec/fixtures"
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/bucket_with_logging_and_tflife/*"],"#{Dir.pwd}/spec/fixtures", )
    end
    ...
    it 'Verify terraform plan count is successful' do
      expect { terraform_plan }.to output(/Plan: 3 to add, 0 to change, 0 to destroy./).to_stdout_from_any_process
    end

    describe 'Verify terraform plan for example bucket_with_logging_and_tflife' do
      before (:all) do
        terraform_plan_output
        @output = terraform_show
      end

      it 'Verify terraform plan for example bucket_with_logging_and_tflife is successful' do
        expect( @output ).to match(/\+ module.bucket_with_logging_and_tflife.aws_s3_bucket.bucket/)
        ...
        expect( @output ).to match(/bucket:\s+"[0-9]{12}-bucket-with-logging-and-tflife-example"/)
        ...
        expect( @output ).to match(/tags.Name:\s+"bucket-with-logging-and-tflife"/)
        expect( @output ).to match(/tags.global_tag:\s+"example"/)
        expect( @output ).to match(/tags.s3_encryption_type:\s+"AES256"/)
        expect( @output ).to match(/tags.s3_feature_flags:\s+"tflife:logging"/)
        ...
        expect( @output ).to match(/\+ module.bucket_with_logging_and_tflife.aws_s3_bucket.log_bucket/)
        expect( @output ).to match(/id:\s+<computed>/)
        expect( @output ).to match(/acceleration_status:\s+<computed>/)
        expect( @output ).to match(/acl:\s+"log-delivery-write"/)
        expect( @output ).to match(/arn:\s+<computed>/)
        expect( @output ).to match(/bucket:\s+"bucket-with-logging-and-tflife-logs-example"/)
        expect( @output ).to match(/bucket_domain_name:\s+<computed>/)
        expect( @output ).to match(/force_destroy:\s+"false"/)
        expect( @output ).to match(/hosted_zone_id:\s+<computed>/)
        expect( @output ).to match(/region:\s+<computed>/)
        ...
        expect( @output ).to match(/\+ module.bucket_with_logging_and_tflife.aws_s3_bucket_policy.bucket_policy/)
        expect( @output ).to match(/id:\s+<computed>/)
        expect( @output ).to match(/bucket:\s+"\${aws_s3_bucket.bucket.id}"/)
        expect( @output ).to match(/policy:\s+"{\\n  \\"Version\\": \\"2012-10-17\\",\\n  \\"Statement\\": \[\\n  {\\n      \\"Sid\\": \\"DenyInsecureCommunications\\",\\n      \\"Effect\\": \\"Deny\\",\\n      \\"Action\\": \\"s3:\*\\",\\n      \\"Resource\\": \\"arn:aws:s3:::[0-9]{12}-bucket-with-logging-and-tflife-example\/\*\\",\\n      \\"Principal\\": \\"\*\\",\\n      \\"Condition\\": {\\n        \\"Bool\\": {\\n          \\"aws:SecureTransport\\": \\"false\\"\\n        }\\n      }\\n    }\\n  \]\\n}\\n"/)
      end
    end
