...
describe 'vpc_with_endpoint_interface' do
  ...
  context "config" do
    before (:all) do
      ...
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/vpc_with_endpoint_interface/*"],"#{Dir.pwd}/spec/fixtures", )
    end
    ...
    describe 'Verify terraform plan output' do
      ...
      it 'Verify terraform endpoint interface output' do
        expect( @output ).to match(/module.vpc-endpoint-interface.aws_vpc_endpoint.interface/)
        expect( @output ).to match(/private_dns_enabled:\s+"true"/)
        expect( @output ).to match(/service_name:\s+"com.amazonaws.us-east-1.kms"/)
        expect( @output ).to match(/vpc_endpoint_type:\s+"Interface"/)
      end
    end
  end
end
