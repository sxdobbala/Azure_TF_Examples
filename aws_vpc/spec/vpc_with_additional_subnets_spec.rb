...
describe 'vpc_with_additional_subnets' do
  ...
  context "config" do
    before (:all) do
      ...
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/vpc_with_additional_subnets/*"],"#{Dir.pwd}/spec/fixtures", )
    end
    ...
    describe 'Verify terraform plan output' do
      ...
      it 'Verify terraform plan output' do
        ...
        expect( @output ).to match(/cidr_block:\s+"10.3.0.0\/16"/)
        ...
        expect( @output ).to match(/triggers.list_of_cidr_block_for_public_subnets:\s+"10.3.[0-9][0-9].0\/20"/)
      ...
      it 'Verify terraform additional subnets' do
        expect( @output ).to match(/module.rds-subnets.aws_subnet.private\[[0-2]\]/)
        expect( @output ).to match(/null_resource.private-cidr-helper\[[0-2]\]/)
        expect( @output ).to match(/module.rds-subnets.aws_network_acl.private/)
        expect( @output ).to match(/module.rds-subnets.aws_route_table.private\[[0-2]\]/)
        expect( @output ).to match(/module.rds-subnets.aws_route_table_association.private\[[0-2]\]/)
        expect( @output ).to match(/module.rds-subnets.aws_vpc_endpoint_route_table_association.private_s3\[[0-2]\]/)
      end
