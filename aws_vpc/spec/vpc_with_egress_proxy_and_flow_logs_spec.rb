...
describe 'vpc_with_egress_proxy_and_flow_logs' do
  ...
  context "config" do
    before (:all) do
      ...
      FileUtils.cp_r(Dir["#{Dir.pwd}/examples/vpc_with_egress_proxy_and_flow_logs/*"],"#{Dir.pwd}/spec/fixtures", )
    end
	...
	expect( @output ).to match(/module.egress_proxy.aws_security_group.proxy_elb/)
	...
	expect( @output ).to match(/aws_security_group_rule.private_egress/)
	expect( @output ).to match(/aws_security_group_rule.allow_private_internet_traffic/)
	expect( @output ).to match(/aws_instance.example-web/)
	expect( @output ).to match(/data.template_file.instance_user_data/)
	...
	expect( @output ).to match(/source_security_group_id:\s+"\${module.vpc.vpc_private_sg_id}"/)
      end
