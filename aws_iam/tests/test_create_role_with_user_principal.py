...
class TestRoleCreationWithUserPrincipals():

    # Captured Terraform output
    tf_output_file = 'examples/create_role_with_user_principal/create_role_with_user_principal.json'     
    
    with open(tf_output_file) as tf_output_file_obj:
        tf_output = json.load(tf_output_file_obj) 

    # Extract expected values
    expected_user = tf_output['user']['value']
    expected_role = tf_output['role']['value']
    expected_principal = tf_output['user_principal']['value']
    ...
    def test_user_exists(self):
        actual_user = self.iam_client.get_user( UserName = self.expected_user )
    ...
    def test_principal_exists(self):
        ...
        actual_principal = actual_role['Role']['AssumeRolePolicyDocument']['Statement'][0]['Principal']['AWS']
