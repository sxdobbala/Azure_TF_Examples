resource "azurerm_public_ip" "publicip03" {
  name                                                 = "${module.random_name.name}-db"
  resource_group_name                                  = "${var.resource_group_name}"
  location                                             = "${var.location}"
  sku                                                  = "Standard"
  public_ip_address_allocation                         = "Static"
  domain_name_label                                    = "${module.random_name.name}db"
...
module "nic03" {
  ...
  ip_configuration_private_ip_address                  = "10.0.3.11"
  ip_configuration_public_ip_address_id                = "${azurerm_public_ip.publicip03.id}"
  tags                                                 = "${var.global_tags}"
}
...
resource "azurerm_virtual_machine" "vm03" {
  ...
  location                                             = "${var.location}"
  resource_group_name                                  = "${var.resource_group_name}"
  network_interface_ids                                = ["${module.nic03.id}"]
  vm_size                                              = "${var.vm_size}"
  tags                                                 = "${var.global_tags}"
