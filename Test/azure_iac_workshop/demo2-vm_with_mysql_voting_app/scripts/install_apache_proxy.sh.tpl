#!/bin/bash -x
#
# Run as root - installs apache (with LetsEncrypt) and configures basic reverse proxy (redirects http->https offloading SSL with LetsEncrypt, proxies to http backend app server)
#
# Variables:
...
if [ -e "$$APTGET" ]; then
  ...
  a2dismod -f auth_basic status authn_core authn_file autoindex setenvif negotiation dir
