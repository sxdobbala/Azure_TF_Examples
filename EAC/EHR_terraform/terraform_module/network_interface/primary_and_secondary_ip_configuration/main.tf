locals {
  ...
  secondary_ip_configuration_private_ip_address_allocation = "${lookup(local.private_ip_address_allocation_types, var.secondary_ip_configuration_private_ip_address_allocation, "Dynamic")}"
...
// Network Interfaces support one or more ip_configuration blocks.
// Currently there isn't a way to easily define multiple blocks like this.
// See this issue: https://github.com/hashicorp/terraform/issues/7034.
// So at this time will have at most two ip_configuration blocks.
...
resource "azurerm_network_interface" "network_interface" {
  ...
  ip_configuration {
    ...
    subnet_id                               = "${var.secondary_ip_configuration_subnet_id}"
    private_ip_address                      = "${var.secondary_ip_configuration_private_ip_address}"
    private_ip_address_allocation           = "${local.secondary_ip_configuration_private_ip_address_allocation}"
    public_ip_address_id                    = "${var.secondary_ip_configuration_public_ip_address_id}"
    load_balancer_backend_address_pools_ids = ["${var.secondary_ip_configuration_load_balancer_backend_address_pools_ids}"]
    load_balancer_inbound_nat_rules_ids     = ["${var.secondary_ip_configuration_load_balancer_inbound_nat_rules_ids}"]
    primary                                 = "false"
  }
}
