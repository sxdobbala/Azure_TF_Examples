...
variable "x509_subject" {
  description = "x509 certificate properties is optional, but if used, all 3 are required.  If subject is empty string this module does not use x509 properties"
  type = "string"
  default = ""
}
# default to "normal" certificate (i.e. type used for a HTTPS website)
variable "x509_key_usage" {
  description = "List of allowed uses for this certificate - use caution when changing this from defaults"
  type = "list"
  default = [ "digitalSignature", "keyEncipherment" ]
}
variable "x509_validity_months" {
  description = "Certificate validity (in months). Must be a minimum of 1"
  type = "string"
  default = "12"
}
