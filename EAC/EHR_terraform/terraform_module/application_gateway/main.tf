locals {
  app_gateway_name = "${length(var.app_gateway_name) == 0 ?
                format("%s-%s",
                  length(var.app_gateway_name_space) == 0 ?
                    format("%s", "default_app_gateway" )
                    :
                    format("%s-%s", "default_app_gateway", var.app_gateway_name_space)
                )
                :
                format("%s", length(var.app_gateway_name_space) == 0 ?
                  format("%s", var.app_gateway_name)
                  :
                  format("%s-%s", var.app_gateway_name, var.app_gateway_name_space)
                )
              }"
...
resource "azurerm_application_gateway" "appgateway_publicip_with_waf_unlimited_ssl" {
  ...
  frontend_port              = "${var.app_gateway_fortend_port_details}"
